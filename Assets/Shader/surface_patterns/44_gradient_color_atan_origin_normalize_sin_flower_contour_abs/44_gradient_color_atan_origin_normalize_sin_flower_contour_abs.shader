Shader "patterns/44_gradient_color_atan_origin_normalize_sin_flower_contour_abs"
{
    Properties
    {
        ...
        _StrengthMultiplier("StrengthMultiplier", Range(1.0, 10.0)) = 1.0
    }
    SubShader
    {
        Tags
        ...
        Pass
        {
            ...
            half4 frag (v2f i) : SV_Target
            {
                ...
                // Step 1: 中心からの距離を計算 ( 中心 0 から 外側にかけて 1 へと値が伸びる )
                float pixelDistance = distance(i.uv, float2(_OriginX, _OriginY));
                // Step 2: 実際の距離と理想的な半径の差を計算 ( 差が0に近いほど線に近い )
                ...
                half4 col = half4(absoluteDifference * _StrengthMultiplier, absoluteDifference * _StrengthMultiplier, absoluteDifference * _StrengthMultiplier, 1.0);
                return col;
            }
