Shader "patterns/31_gradient_color_center_point_smoothstep"
{
    Properties
    {
        ...
        _StepThreshold("CutoffThreshold", Range(0.1, 0.2)) = 0.1
        _SmoothThreshold("SmoothThreshold", Range(0.0, 0.1)) = 0.05
    }
    SubShader
    {
        Tags
        ...
        Pass
        {
            ...
            */
            half4 frag (v2f i) : SV_Target
            {
                ...
                // smoothstep(0.09, 0.1, strength) の時
                // strength = 0.09 のとき → 0.0
                // strength < 0.09 → 0.0 (完全に黒)
                // 0.09 ≤ strength ≤ 0.1 → 0.0〜1.0 (滑らかに変化)
                // strength > 0.1 → 1.0 (元の値を保持)

                strength = smoothstep(_SmoothThreshold, _StepThreshold, strength) * strength;
